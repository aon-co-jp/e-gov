...
> This notice is shown every time, whether you use the website or add the
> LINE account as a friend. Electronic notarisation and legally binding
> electronic contracts (property sale/lease) have not been implemented
> yet, pending formal regulatory approval.
...
- **Tiered identity verification**: verification strength scales with the
  value/importance of the transaction (caller-ID + call-back → email OTP
  → My Number card NFC scan via smartphone).
- Aims for paperless online applications and a one-stop experience across
  multiple government agencies/local authorities.
...
- **Real-estate investment and AI home-builder**: an AI suggests floor
  ...
  not fuelling speculative capital inflow into property.
- The initial phase runs without real stock, monetised via advertising
  revenue as a demonstration of the AI chat-commerce UX.

## Reference models studied

- Estonia's e-Estonia / X-Road (decentralised data-exchange backbone)
- Azerbaijan's ASAN xidmet (one-stop service centres, mobile outreach)
...
triggered by keywords such as "apply", "buy", "trade credit", "land
search"). A QR code for adding the friend will be published once the
LINE Official Account's Basic ID has been issued.

## Auto-displayed from GitHub

The homepage fetches this repository's `README.md`, `CLAUDE.md`, and
`PORTING.md` live from GitHub and renders them GitHub-style (with a
toggle to a `.rs`-style rendering).

## Tech stack

Rust + [Poem](https://github.com/poem-web/poem). No database dependency,
single self-contained binary. Same stack as `aruaru-tokyo`/`karu.tokyo`.
...
- [aruaru-db](https://github.com/aon-co-jp/aruaru-db) — future database connection candidate
